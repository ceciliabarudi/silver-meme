...
context('Pet project', () => {
    describe('Operating on resources', function () {
        ...
        it('should be able to delete animal', function () {
            cy.visit('/')
            cy.request('DELETE', '/animals/3')
                .then((response) => {
                    expect(response.status).to.eq(200)
                })
            cy.visit('/animals');
            cy.contains('Dog, Cat, Raccoon, Duck, Narwhal')
        });
    });
})